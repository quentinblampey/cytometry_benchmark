...
def compute_BTLu(y_lpt, nbr_id, n_sample, n_type, n_lpt):
    ...
    BT = scipy.sparse.csc_matrix(
        dict_2_cs(cor_BT), shape=(n_sample, n_type), dtype=np.float
    )
    Lu = scipy.sparse.csc_matrix(
        dict_2_cs(cor_Lu), shape=(n_sample, n_sample), dtype=np.float
    )
    return BT, Lu
