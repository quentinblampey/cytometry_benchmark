...
def main(filename, k):
    ...
    if filename is None:
        handgatedFileinfo = (
            f"poised/{batch}/Handgated.csv"  ## hand-gated cells to be used for training
        )
        LiveFileinfo = f"poised/{batch}/LivecellsTraining.csv"  ## All Live cells of the samples used for handgating (i.e. training); these samples will also be labelled
        unlabelledDataset = f"poised/{batch}/Livecells.csv"  ## All Live cells to be tested for annotation, i.e. for cell type identification
    else:
        handgatedFileinfo = f"poised_{filename}/{batch}/Handgated.csv"  ## hand-gated cells to be used for training
        LiveFileinfo = f"poised_{filename}/{batch}/LivecellsTraining.csv"  ## All Live cells of the samples used for handgating (i.e. training); these samples will also be labelled
        unlabelledDataset = f"poised_{filename}/{batch}/Livecells.csv"  ## All Live cells to be tested for annotation, i.e. for cell type identification
    relevantMarkers = [
        "CD16",
        "IFNg",
        "CD69",
        ...
        "CD56",
        "CLA",
    ]  ## [Mandatory] lineage markers used for hand-gated; based on column names in FCS/CSV file
    if filename is None:
        outdir = f"poised_out_None_{batch}"  ##  Mandatory when loadModel=False] any name of choice. If directory will hold saved session and labelled CSV file.
    else:
        outdir = f"poised_out_{filename}_{batch}"  ##  Mandatory when loadModel=False] any name of choice. If directory will hold saved session and labelled CSV file.
