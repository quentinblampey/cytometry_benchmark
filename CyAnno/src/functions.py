...
from scipy import mean, nanmean, nanstd, spatial, stats
...
def method0(
    ...
    ct2indx = {
        CT: (i + 1) for i, CT in enumerate(list(np.unique(labels)))
    }  ## +1 ensures that first celltype will not get 0 as cell typen name ; this is helpful in SVM
    ...
    indx2ct = {
        value: key for key, value in ct2indx.items()
    }  ## get celltype name back from its index
    print("Reading handgated cells...")
    PIDs = []
    if Findungated:
        PIDs = getPIDnames(
            md
        )  ## list of sampleIDs that are needed to be analyzed for ungated cell identification
        UNKcount = dict(
            zip(sample_ids, [0] * md.shape[0])
        )  ## a dict in which key is sample_id and value is zero which will be updated by the total number of ungated cells present in this sample ID
    for c, f in enumerate(inputs):
        if os.path.exists(f):
            ...
            cellType = ct2indx[
                labels[c]
            ]  ## instead of actual celltypr name use a number to
            if match:  ## if file is FCS
                ...
                print(
                    str(
                        "[ERROR] Line: "
                        + c
                        + "- Unknown File format, must be CSV or FCS only!!!"
                    )
                )
                fl.write(
                    "[ERROR] Line: "
            ...
            if Findungated is True and (
                key in PIDs
            ):  ## for these samples ungated cells needs to be calculated so we are storing their all gated cells expression values
            ...
            if Findungated is True and (
                labels[c]
                in ["Ungated", "Unknown", "Unclassified", "Other", "Undefined"]
            ):  ## this will be used later to plot the proportions of ungated cells present in each sample used in training dataset
                UNKcount[key] = (
                    UNKcount[key] + exp.shape[0]
                )  ## updating the number of ungated cells observed in this sample_id, if found
            TotalCount.update(
                {key: TotalCount.get(key, 0) + exp.shape[0]}
            )  ## Total cells found in each sample_id
            indxrm = None
            if filterCells == True:
                indxrm = filterCells(
                    exp
                )  ## this will give indexes to remove bad quality cell;
                exp.drop(
                    indxrm, axis=0, inplace=True
                )  ## this will remove the bad quality cells
            if (
                cellType in ct2expr.keys()
            ):  ## checking if this celltype has already been used for gathering expression from other sample_id
                exp = pa.concat(
                    [ct2expr[cellType].X, exp]
                )  ## concatenate (rbind) with existsing expression dataframe
    ...
    for ct, expr in ct2expr.items():
        if (
            expr.X.shape[0] >= cellCount
        ):  ## removing celltypes with poor cell count; less equal to than cellCount
            tmp[ct] = expr
        else:
            print(
                "[Warning]",
                str(indx2ct[ct]),
                "has low number of cells ("
                + str(expr.X.shape[0])
                + " cells). To include this cell type reduce the value 'cellCount' argument",
            )
            fl.write(
                "[Warning]"
                + str(indx2ct[ct])
                + "has low number of cells ("
                + str(expr.X.shape[0])
                + " cells). To include this cell type reduce the value of 'cellCount' argument\n"
            )
    ct2expr = tmp
    fl.close()
    # if umap == True:
    ...
    if (
        Findungated is True and sum(UNKcount.values()) > 1
    ):  ## plotting the proportion of ungated samples in each sample_id from which training dataset arose
        X = [k for k in TotalCount.keys()]  ## all sample_ids used in training dataset
        T = [
            TotalCount[k] for k in TotalCount.keys()
        ]  ## total number of cells in all sample_ids used in training dataset
        Y = [
            UNKcount[k] for k in TotalCount.keys()
        ]  ## number of ungated cells observed each of the sample_id used in training dataset
        P = [
            (Y[i] / T[i]) * 100 for i in range(len(Y))
        ]  ## proportion of ungated cells observed
    ...
    for b in batches:
        ...
        for t in TimePoints:
            ...
            for s in Stims:
                ...
                selectedPID = np.random.choice(
                    PIDs, 1
                ).tolist()  ## this is the randonly selected patient that will be used for this batch b for time t with stim s ## suppose there are multiple samples in a PeaStim in agiven batch for time t; then it picks all cell types of only one sample randomly
...
def method1(
    ...
    if (
        ungated is True and len(train.l) > 0
    ):  ##Check if PIDs are non-zeros, i.e. there are sample IDs that are required to be scanned for ungated cellsn
        fnd = list(set(list(train.l)) & set(list(sample_ids)))  ## check if
        if len(fnd) != len(
            train.l
        ):  ## check if all of these samples exists as live cells
            notfound = list(
                set(train.l) - set(fnd)
            )  ## some or all samplesIDs do not have live ungated datasets available. find their sampleIDs
            print(
                "Error!!! The live cells belonging to these sampleIDs: "
                + str(notfound)
                + " not found. These samples are required for finding their ungated cell population. Either set ungated=False or read manual"
            )
            exit
    ...
    for c, f in enumerate(inputs):
        if os.path.exists(f):
            ...
            if match:  ## if file is FCS
                ...
                print(f)
            if (
                "labels" in exp.columns
            ):  ## when the test CSV/FCS file already have labels for every cell ; the column name must be labels
                CT = exp.loc[
                    :, "labels"
                ]  ## this is those cases when labels are already available for test dataset; good for benchmarking
                exp.drop(
                    ["labels"], axis=1, inplace=True
                )  ## remove this from expression matrix as we will be filling it with predicted values
    ...
    if (
        ungated is True and len(train.l) > 0
    ):  ## there are sample_IDs for which ungated cells have to be calculated
        train = labelUngated(
            test, train, relevantMarkers
        )  ## train.z is expression values of gated cells in train.l sample_ids from which ungated cells needs to be calculated
    if (
        normalizeCell is True
    ):  ## normalize only after ungated cells are predicted; if required
...
def preprocess(test, relevantMarkers, cofactor):
    for key, value in test.items():
        ...
        value.X[
            value.X < 0
        ] = 0  ## if expression is negative then make it zero as cell cannot be negatively expressed also it will create problem in normalizarion
        if len(value.X.columns) == len(relevantMarkers):
            ...
            expr1 = value.X.drop(
                columns=relevantMarkers, inplace=False
            )  ## separating out other columns, if any
            expr2 = value.X.loc[
                :, relevantMarkers
            ]  ## normalizing only the expression of relevant markers
            exp = np.arcsinh((expr2 - 1.0) / cofactor)
            test[key].X = pa.concat(
                [exp, expr1], axis=1
            )  ## again adding the columns excluded from normalization
        else:
            print(
                "[ERROR] Number of relevant markers are greater than markers in expression matrix of live cells"
            )
            os.exit(0)
    return test
...
def labelUngated(test, train, relevantMarkers):
    ...
    for sample_id in PIDs:
        ...
        xb = np.ascontiguousarray(
            np.float32(allLiveCells.values)
        )  ## making faiss database of epoch-specific marker expression matrix
        index.add(xb)  ## indexing the database
        xq = np.ascontiguousarray(
            np.float32(cleangated.values)
        )  ## getting from the dict ; should be panda data frame of landmark cells with column in same order as orignal expr dataframe ## for each celltype get the landmarks cells (as expression values of all markers from 'eset') ##
        D, I = index.search(
            xq, 1
        )  ##  ## get neighbours of all the landmarks cells for a given cell type ## I variable has rowindexes of cells that should belong to same distribution to the
        indexes = list(
            np.unique(I.ravel())
        )  ## these are the indexes of gated population in all live cells
        ...
        if percentError > 2.0:
            print(
                "*** Too Much error rate in this sample ID. Ungated cells will not be considered for analysis"
            )
        else:
            if key in train.X:
                train.X[0] = Sample(
                    X=pa.concat([train.X[0].X, pa.DataFrame(Ungatedcells)], axis=0)
                )
            else:
...
def plotUMAP(
    ct2expr, ct2indx
):  ## upto 30 different celltype can be plotted, if more cell types are there then add the number of colors accordingly
    dataSample = ReduceDFbyLabel(ct2expr, 3000)
    embedding = umap.UMAP(n_neighbors=15, min_dist=0.25).fit_transform(
        dataSample.X.values
    )
    target = np.array([ct2indx[x] for x in dataSample.y.values.ravel()])
    df = pa.DataFrame(
        {"UMAP1": embedding[:, 0], "UMAP2": embedding[:, 1], "Cell_Type": list(target)}
    )
...
def filterCells(
    exp,
):  ##TO DO LIST : instaed of removing it should just give index to be removed in each matrix
    ...
    remove = np.unique(
        merged
    )  ## these cell indeces have atleast one marker that is outlier and these cells will be discarded from analysis
    per_cells_removed = (len(remove) / exp.shape[0]) * 100
    print("Level 1: Cell can be removed ", per_cells_removed, "%(", len(remove), ")")
    min_max_scaler = (
        preprocessing.MinMaxScaler()
    )  ## scaling column wise expression between 0 to 1
    x_scaled = min_max_scaler.fit_transform(exp)
    x = pa.DataFrame(
        x_scaled, columns=exp.columns
    )  ## converting numpy array to panda df
    info = x.sum(axis=1)  ## sum of each row or cell(info score)
    threshold = (
        0.05  ## INFOi threshold ## cells with score less than this value have no use
    )
    to_remove = (
        []
    )  ## this list will hold indexes of all those cells that are needed to be removed
    tmp = info[
        info <= threshold
    ].index.tolist()  ## these cells have INFO score very less and these cells contribute nothing so removed
    to_remove.append(tmp)
    df = x.max(axis=1)
    tmp = df[(info * 0.5) < df].index.tolist()
    to_remove.append(tmp)
    merged = list(
        itertools.chain(*to_remove, remove.tolist())
    )  ## merging the index from level 1 of filtering and level2
...
def countEpoch(blockInfo, allowed):
    ...
    for key, value in blockInfo.items():
        blocksCount = len(
            blockInfo[key]
        )  ## blockInfo is hash of hash so its value is also an hash whose length is the
        counts.append(blocksCount)
    epoch = (
        max(counts) * allowed
    )  ## each block in the sample with maximum number of blocks must cover 'allowed' iterations to generate reliable probability
    return epoch
...
def makePrediction(E, M, lda, CellTypes, postProb):
    ...
    pred = lda.predict(X)
    pred_prob = lda.predict_proba(
        X
    )  ## Posterior probability lower threshold, below which the prediction will be 'unknown',
    max_prob = np.amax(
        pred_prob, axis=1
    )  ## maximum probability associated with each cell (row) for a given cell type among all celltypes
    ...
    unk_index = np.nonzero(
        max_prob < postProb
    )  ## getting index of cells when maximum post. prob of cell to be associated with a given celltype is less than threshold
    pred[
        unk_index
    ] = "Unknown"  ## replacing the assigningment of cells as "unknwon" if their post prob. is less than threshold
    unassignedCellsProp = (
        np.shape(unk_index)[0] / np.shape(pred)[0]
    ) * 100  ##%age of cells that are likely to be of unknown category
...
def fitGMM(
    ct2expr, Maxlimit, totalCells, indx2ct
):  ## this will create a separate 2-component GMM model for each cell type; this will help us in shortlisting bad neighbours preicted by faiss
    # Maxlimit = 100000 ## Maximum number of cells to make GMM fitting per cell type
    models = {}
    freqCT = {}
    print(
        "Fitting GMM and finding Landmarks (may be time consuming for some cell types) "
    )
...
def farthest_search(df, top):
    ...
    for _ in range(top - 1):
        ...
        out += list(
            df.index[z]
        )  ## getiing the orignal rowname/index of those indexes found to be farthest from teach other in this run
...
def checkLM(TP1, TP2):
    ...
    if TP2 == None:
        ...
        if (
            len(common) / len(TP2) >= 0.98
        ):  ## more than 95% of common TP exits within TP1; why would i NEED TP2 THEN; its a repetitive landmark. i dont need it
...
def run_CATboost(X_train, y_train, X_test, y_test, ct, threads):
    ...
    catboostmod = CatBoostClassifier(
        depth=10,
        learning_rate=0.1,
        l2_leaf_reg=3,
        random_seed=164530,
    ...
    print(
        "CATBoosting for cell type: "
        + str(ct)
        + " // Precision : {:.2f}".format(accu1)
        + " // F1-score : {:.2f}".format(accu11)
    )
    return Sample(X=catboostmod, y=accu11)


def runEnsemble(
    X_train, y_train, X_test, y_test, my_test, ct, obj0, tree, plotlogLoss, SVM, threads
):
    eval_set = [(X_train, y_train), (X_test, y_test)]
    ...
    print("**********************************")
    print(
        "Ensemble Model Result cell type: "
        + str(ct)
        + " // Precision : {:.2f}".format(ensemblePrecision)
        + " // F1-score : {:.2f}".format(ensembleF1)
    )
    print("**********************************")
    return ensemble
...
def CellTypeAcc(y_test_pred, y_test, text, y_test_Postprob, indx2ct):
    ...
    for ct in list(mlist):
        if ct != 0:
            ...
            common = list(
                set(ind1).intersection(ind2)
            )  ## true positive prediction; the indexes that are predicted to be this cell type (in y_pred) are also the same cell type in y_test
            common2 = list(
                set(ind11).intersection(ind22)
            )  ## true negative prediction; the indexes that are predicted to be this cell type (in y_pred) are also the same cell type in y_test
            probs = probs.iloc[
                common, :
            ].values  ## prosterior probs observed in TP pedictions by LDA or QDA
            errorRate = (len(ind1) - len(common)) / len(ind1)
            Dplot[ct] = {
                "thesh": np.quantile(probs, 0.01),
...
def AllvsAllQDA(X_train, y_train, X_test, y_test, indx2ct, relevantMarkers, UnkKey):
    ...
    y_train = y_train[
        y_train != UnkKey
    ]  ## here we are removing unkown cells from training. though we will keep them for testing (as randomly scattered among gated cell types)
    ...
    ax2 = df.plot.scatter(
        x="Log population Size", y="F1-score", c="F1-score", colormap="viridis"
    )
...
def plot_LM_cells_in_PCA(Xi, rows, col, name):
    ...
    df = pa.concat(
        [
            pa.DataFrame(PCs, columns=["PC1", "PC2"]),
            pa.DataFrame(labs, columns=["label"]),
        ],
        axis=1,
    )
...
def getCenter2(data, name, nlandMarks, plot):
    ...
    edges = alpha_shape(
        data, alpha=0.80, only_outer=True
    )  ## those cells that lie outside of the PC cluster
    edgeslist = [[i] + [j] for i, j in edges]
    desiredIndex = list(chain(*edgeslist))
    warnings.filterwarnings("ignore")
    myPDF, axes, z = fast_kde(
        data[:, 0], data[:, 1], sample=True
    )  ## the modified KDE downloaded from https://gist.github.com/joferkington/d95101a61a02e0ba63e5
    z = pa.DataFrame(stats.zscore(z))
    quants = [
        i for i in np.arange(0.98, 0.2, -0.12)
    ]  ###  [0.99,0.89,0.79,0.69,0.59,0.49,0.39,0.29,0.19] ## qualtile ranges
    threshs = [
        np.quantile(z, q) for q in quants
    ]  ## maximum value of Kernel density within each qantile range
    ...
    if plot == True:
        ...
        ax = test0.plot.scatter(
            x="x", y="y", c="z", colormap="viridis"
        )  ## if you are running the above three lines; hash this line
        plt = ax.get_figure()
        plt.savefig(name)
    ...
    for cellType in indx2ct.keys():
        if cellType != UnkKey:
            ...
            if Xi.shape[0] > 100:
                if (
                    Xi.shape[0] > 80000
                ):  ## maximum 80K cells per cell type for LM identification
                ...
                LMind = getCenter2(
                    data=data, name=name, nlandMarks=nlandMarks, plot=LandmarkPlots
                )  ##LandmarkPlots: True or False if you want to have scatter plot of PCs with dense clsuetrs
                Testcellindex = np.where(y_test == cellType)[
                    0
                ]  ## index of this cellype in this test dataset
                neighbours = len(Testcellindex)
                cells_DF = Xi[
                    LMind.X, :
                ]  ## expression matrix of LM cells only ## ******
                ###################################################
                xq = np.ascontiguousarray(
                    np.float32(cells_DF)
                )  ## searching LM cells nearest neighbor in test set
                D, I = index.search(
                    xq, neighbours
                )  ##  ## get neighbours of all the landmarks cells for a given cell type ## I variable has rowindexes of cells that should belong to same distribution to the
                ...
                for indx in range(
                    len(I)
                ):  ## read all the predicted neighbours of every LM one by one; reading the output of landmarks one by one
                    TruePos = set(I[indx]) & set(
                        Testcellindex
                    )  ## common indxes between predicted indexes for this LM and orignal TP indexes
                    if (
                        len(TruePos) / len(Testcellindex)
                    ) >= 0.09:  ### atleast 0.9% of indexes are TP; otherwise I dont need that LM
                ...
                found = (
                    {}
                )  ## for each of celltype index found ; how many LMs can find  this index
                rows = []
                densities = []  ## ******
                for indx in range(len(I) - 1):
                    TP1 = comparison[
                        indx
                    ]  ## output of this landmark prediction; by deafult the very first index will always be used; no matter waht; its the most dense LM
                    if TP1 is not None:
                        # choice = [checkLM(TP1,comparison[indx2]) for indx2 in range(indx+1, len(I))] ##***** ## indexes that are TP predict by subsequent landmark
                        comparison = {
                            indx2: checkLM(TP1, comparison[indx2])
                            for indx2 in range(indx + 1, len(I))
                        }  ## if the first TP1 is better than anyone of the subsequenct LM than replace that subsequent LM value with None; else keep it as such
                        ...
                        rows = rows + [
                            cells_DF[indx].tolist()
                        ]  ## append the marker expression of this LM as list
                        densities = densities + [
                            LMind.y.iloc[indx, 0]
                        ]  ## ****** adding the kernel densities of LM cells
                        found.update({tp: found.get(tp, 0) + 1 for tp in TP1})
                    if (
                        len(found.keys()) / neighbours
                    ) > 0.99:  ## the current landmark has already predicted more than 98% of expected indexes
                        break  ## if all the expected neigbours are already predicted; do not go ahead
                acc = (len(found.keys()) / neighbours) * 100
                print(
                    "Achieved "
                    + str(acc)
                    + "% of cells in Cell Type "
                    + str(ct)
                    + " with "
                    + str(len(rows))
                    + " / "
                ...
                if acc <= 50:
                    rows = cells_DF[:, :]
                    f.write(
                        "[Note] Achieved less than 50% of cells in Cell Type "
                        + str(ct)
                        + ", taking all landmark candidates\n"
                    )
                    print(
                        "[Note] Achieved less than 50% of cells in Cell Type "
                        + str(ct)
                ...
                Ddensity[
                    cellType
                ] = densities  ## list of LM densities (for each LM cell) in each cell type
            else:
                rows = Xi[:, :]
                f.write(
                    "[Note] Less than 100 cells in Cell Type "
                    + str(ct)
                    + ", taking all cells as landmarks\n"
                )
                print(
                    "[Note] Less than 100 cells in Cell Type "
                    + str(ct)
            ...
            for s in range(len(rows)):
                indexes.append(
                    "LM" + str(s)
                )  ## adding rownames to landmark cells to probe them later ,e.g. LM0; LM1;LM2... dependng upon number of landmark cells
            landmarks[cellType].index = indexes
    f.close()
    return Sample(
        X=landmarks, y=Ddensity
    )  ## a dict of cell type and best landmarks (panda df) with expression values
...
def runPCA(X):
    ...
    pca = PCA(
        n_components=2
    )  ## since these are pre-sorted cells (hand-gated), we donot expect much variance
...
def svc_param_selection(X, y, nfolds, ct, threads, X_test, y_test):
    ...
    param_grid = {
        "base_estimator__C": [0.1, 0.01, 0.001],
        "base_estimator__gamma": [1, 0.1, 0.01],
        "base_estimator__kernel": ["rbf"],
    }
    if (
        X.shape[0] > 15000
    ):  ## too many cells for svm to complete under practical time frame.
...
def run_multi_layer_perceptron(X_train, y_train, X_test, y_test, threads, ct):
    ...
    accu11 = f1_score(y_test, y_test_pred, average="weighted")
    print(
        "Multi-Layer-Percepteron for cell type: "
        + str(ct)
        + " // Precision : {:.2f}".format(accu1)
        + " // F1-score : {:.2f}".format(accu11)
    )
...
def run_XGboost(X_train, y_train, X_test, y_test, ct, plotlogLoss, threads):
    ...
    xgbmdl = XGBClassifier(
        ...
        y_train,
        early_stopping_rounds=10,
        eval_metric=["error@0.60", "logloss"],
        eval_set=eval_set,
        verbose=True,
    )
...
def QDAboost(
    ...
    if method == "m" or method == "b" or method == "e":
        ...
        mlpout = run_multi_layer_perceptron(
            X_train, y_train, X_test, y_test, threads, ct
        )
        mlpMod_ = mlpout.X
    ...
    Time = pa.DataFrame(
        {
            "Celltype": str(ct),
            "XGboost": [TimeTakenXGB],
            "MLP": [TimeTakenMLP],
            "SVM": [TimeTakenSVM],
            "CellCount": X_train.shape[0],
        }
    )
...
def trainHandgated(
    ...
    skf = StratifiedShuffleSplit(
        n_splits=1, test_size=0.4, random_state=randint(0, 100000)
    )
    models = {}
    freqCT = {}
    obj0 = None
    ...
    if (
        (method == "x") or (method == "e") or (method == "m") or (method == "b")
    ):  ## if method is not multi-class LDA ; else take this above model
        obj = findLandmarks(
            X_train,
            y_train,
            X_test,
            ...
            UnkKey,
            ProjectName,
        )  ### identify the best landmarks with non-redundant outcome and good density
        lmarks = obj.X
        ...
        index = faiss.IndexFlatL2(d)  ## initiating faiss database
        xb = np.ascontiguousarray(
            np.float32(ExprSet.X.values)
        )  ## making faiss database of epoch-specific marker expression matrix
        index.add(xb)  ## indexing the database
        Time = pa.DataFrame(
            {"Celltype": [], "XGboost": [], "MLP": [], "SVM": [], "CellCount": []}
        )
        ...
        for CT in ct2expr.keys():
            if CT != UnkKey:  ## Ignoring unknown cells for classification
                ...
                if neighbours < 10:
                    sys.exit(
                        "Error: the number of cells for the cell type "
                        + str(indx2ct[CT])
                        + " is less than 20. Please either remove the celltype from input training FCS files or add more training FCS datasets for this cell type"
                    )
                LMs = lmarks[CT]
                ...
                f.write(
                    "The %age of True Nearest Neighbour for modelling for the cell type "
                    + str(indx2ct[CT])
                    + " is "
                    + str(TrueNeaNeigh)
                    + "% with TP "
                ...
                mY = Y[:]
                Y = Y.applymap(
                    lambda x: 0 if x != CT else 1
                )  # IN labels all celtypes other than this will be zero
                if len(np.unique(Y)) > 1:
                    for train_index, test_index in skf.split(X, Y):
                        ...
                        my_test = mY.iloc[
                            test_index, :
                        ].values.ravel()  ## this has all the classes in test set instead of just binary classes
                        ...
                        models[CT] = QDAboost(
                            ...
                            ProjectName,
                            CT,
                        )  ## output is a sample obj
                        Time = pa.concat([Time, models[CT].p], axis=0)
                        F1stats = pa.concat(
                            [F1stats, pa.DataFrame([models[CT].q])], axis=0
                        )
        ...
        saveObjects(
            ProjectName, obj, models, freqCT, indx2ct, UnkKey, relevantMarkers, method
        )
        f = open(ProjectName + "/output.log", "a")
        ...
        obj = AllvsAllQDA(
            X_train, y_train, X_test, y_test, indx2ct, relevantMarkers, UnkKey
        )
        models = None
        freqCT = None
        Time = None
...
def printACC(
    indx2ct, origLabels, predictedLabels, sampleID, samPre, samF1, samrecall, type1
):
    result = pa.DataFrame(
        {
            "SampleID": [],
        ...
        if ct != "Unknown22":
            ...
            if precision != 2:
                print(
                    ...
                    + "/"
                    + str(len(indx))
                    + " cells is : {:.2f}".format(precision)
                    + " & recall is : {:.2f}".format(recall)
                    + " & F1 score is :  {:.2f}".format(F1)
                )
                print(
                    "###############################################################################"
                )
                tmp = pa.DataFrame(
                    {
                        ...
                        "FP": [FP],
                        "FN": [FN],
                        "CellCount_Exp": [len(indx)],
                        "CellCount_obs": [len(indx2)],
                        "Sampleprecision": [samPre],
                        "SampleF1": [samF1],
                        "SampleRecall": [samrecall],
                        "Type": [type1],
                    },
                    index=[ct],
                )
...
def CelltypePredPerSample(
    ...
    X = np.array(
        expr.loc[:, relevantMarkers]
    )  ## a marker expression matrix for each sample; made by comboining one block from each cell
    f = open(ProjectName + "/output.log", "a")
    ...
    if method != "l":  ## method is not mLDA
        ...
        obj = (
            SelfObject.X
        )  ## a dict; chosen landmark cells (with expression values) for each cell type
        landmarks = obj.X
        freqCT = (
            SelfObject.z
        )  ##  frequency/proportion of a given cell type in hand-gated cells
        d = len(relevantMarkers)  # number of markers
        np.random.seed(1234)
        index = faiss.IndexFlatL2(d)  ## initiating faiss database
        xb = np.ascontiguousarray(
        ...
        for (
            ...
            freqCT.items()
        ):  ## this dict has cell-type as key and expected prop. as value  ##EN : number of neighbours you need; this can be the number of cells expected for a given cell type ## count the number of cells supposed to be there in the dataset ##
            print(".", end="")
            ################## Nearest Neighbourhood approximation #############################
            neighbours = int(EN * len(X))
            xq = np.ascontiguousarray(
                np.float32(landmarks[CT].values)
            )  ## getting from the dict ; should be panda data frame of landmark cells with column in same order as orignal expr dataframe ## for each celltype get the landmarks cells (as expression values of all markers from 'eset') ##
            D, I = index.search(
                xq, neighbours
            )  ##  ## get neighbours of all the landmarks cells for a given cell type ## I variable has rowindexes of cells that should belong to same distribution to the
            indexes = list(
                np.unique(I.ravel())
            )  ## remove the duplicate indexes predicted to be neighbours of all landmark cells
            tmpdf = np.concatenate(
                (landmarks[CT].values, X[indexes]), axis=0
            )  ## the first n cells in this case is landmark cell which is also used in modelling ## first cell is also the cell closest to the query cell in faiss
            ##################### scoring with model ##########
            tmpdf = pa.DataFrame(
                np.delete(tmpdf, slice(landmarks[CT].shape[0]), 0),
                columns=relevantMarkers,
            )  ## removing landmark cells from the cell-type specific exp matrix
            if method == "e":  ## ensemble method
                postProb = ensemble_[CT].z.predict_proba(
                    tmpdf
                )  ## prob. prob to cells using cell type specific SVM+QDA+XGboost ensemble model (soft maximum voting)
                column = np.argmax(
                    ensemble_[CT].z.classes_
                )  ## column number of cell type in the model; .y only for SVM ;; .z only for ensemble ## .l only for one-class QDA and .o only for mult-LDA
            elif (
                method == "b"
            ):  ## best model per cell type from XGboost/Multi-layer-percep/mLDA
                postProb = ensemble_[CT].X.predict_proba(
                    tmpdf
                )  ## prob. prob to cells using cell type specific SVM+QDA+XGboost ensemble model (soft maximum voting)
                column = np.argmax(
                    ensemble_[CT].X.classes_
                )  ## column number of cell type in the model; .y only for SVM ;; .z only for ensemble ## .l only for one-class QDA and .o only for mult-LDA
            elif method == "m":  ## multi-layer perceptron
                postProb = ensemble_[CT].l.predict_proba(
                    tmpdf
                )  ## prob. prob to cells using cell type specific SVM+QDA+XGboost ensemble model (soft maximum voting)
                column = np.argmax(
                    ensemble_[CT].l.classes_
                )  ## column number of cell type in the model; .y only for SVM ;; .z only for ensemble ## .l only for one-class QDA and .o only for mult-LDA]
            elif method == "x":  ## XGboost
                postProb = ensemble_[CT].y.predict_proba(
                    tmpdf
                )  ## prob. prob to cells using cell type specific model
                column = np.argmax(
                    ensemble_[CT].y.classes_
                )  ## column number of cell type in the model
            else:
            ...
            tmp1 = list(
                postProb[:, column]
            )  ## posterior prob. of cell to belong to this cell type using best classifier
            tmp1 = [
                r if r > postProbThresh else 0 for r in tmp1
            ]  ## if post. prob. of cell to be this cell type is <= 0.5 then lets not consider this cell as the CT cell type; make its post prob. 0
            c = Counter(tmp1)
            len(c.keys())
        ...
        unknown = pa.DataFrame(
            [1.0 if r == 0 else 0 for r in rowsum], columns=[0]
        )  ## if the sum of the post. prob. across all cell type for a given cell is zero then make its post. prob. == 1 for "Unknown"cell type
    ...
    if origLabels is not None and len(origLabels) > 2:
        ...
        if calcPvalue:
            Pvalue = calculatePvalue(
                origLabels, predictedLabels, F1
            )  ## calculating p-value of F1 SCORE for this sample
        ...
        df = pa.DataFrame(
            {
                "SampleID": sampleID,
                "CellTypeOrignal": origLabels.values.tolist(),
                "CellTypePredicted": predictedLabels,
        ...
        annotatedExpression = pa.concat(
            [
                expr,
                pa.DataFrame(
                    predictedLabels, columns=["Predictedlabels"], index=expr.index
                ),
            ],
            axis=1,
        )
        ...
        annotatedExpression.to_csv(
            name
        )  ## write best cell type predicted for this sample
    else:
        print("Merging...")
        annotatedExpression = pa.concat(
            [expr, pa.DataFrame(predictedLabels, columns=["labels"], index=expr.index)],
            axis=1,
        )
        name = str(ProjectName) + "/" + str(sampleID) + "_labelled_expr.csv"
        print("Writing file.." + name)
...
def getSet(ct2expr, indx2ct, relevantMarkers, normalize, ProjectName):
    ...
    for key, value in ct2expr.items():
        ...
        if value.X.shape[0] < 10:
            print(
                "Too Low number of cells (< 10) for training the cell type: "
                + str(indx2ct[key])
                + " ... this Cell type ignored for analysis"
            )
            fl.write(
                "Too Low number of cells (< 10) for training the cell type: "
                + str(indx2ct[key])
...
def loadObjects(ProjectName, method):
    if method == "l":
        ...
        indx2ct = load_Object(
            ProjectName + "/others_/indx2ct.PyData", "indexes to cell types"
        )
        UnkKey = load_Object(ProjectName + "/others_/UnkKey.PyData", "Unknown Keys")
        relevantMarkers = load_Object(
            ProjectName + "/others_/relevantMarkers.PyData", "Relevant Markers"
        )
        models = None
        freqCT = None
    else:
        ...
        for CT in freqCT.keys():
            X = load_Object(
                ProjectName + "/models_/C" + str(CT) + "/C1.PyData",
                str(CT) + "..celltype",
            )
            y = load_Object(
                ProjectName + "/models_/C" + str(CT) + "/C4.PyData",
                str(CT) + "..celltype",
            )
            z = load_Object(
                ProjectName + "/models_/C" + str(CT) + "/C3.PyData",
                str(CT) + "..celltype",
            )
            l = load_Object(
                ProjectName + "/models_/C" + str(CT) + "/C2.PyData",
                str(CT) + "..celltype",
            )
            o = load_Object(
                ProjectName + "/models_/C" + str(CT) + "/C5.PyData",
                str(CT) + "..celltype",
            )
            models[CT] = Sample(X=X, y=y, z=z, l=l, o=o)
    return Sample(
        X=obj, y=models, z=freqCT, l=indx2ct, o=UnkKey, p=relevantMarkers, q=method
    )
...
def load_Object(file1, pr):
    ...
    if not os.path.exists(file1):
        sys.exit(
            "[Error while loading session]: File:  " + str(file1) + " doest not exist"
        )
    else:
...
def arcsinetransform(value, relevantMarkers, key):
    ...
    if len(value.columns) == len(relevantMarkers):
        value = np.clip(
            value.loc[:, relevantMarkers], a_min=0, a_max=None
        )  ## if expression is negative then make it zero as cell cannot be negatively expressed also it will create problem in normalizarion
...
def e2b(
    ...
    if not loadModel:
        ...
        UnkKey = 0  ## training set does not have Unknown cells
        for k, v in indx2ct.items():
            if (
                ...
                UnkKey = k
        ExprSet = getSet(
            train.X, indx2ct, relevantMarkers, normalizeCell, ProjectName
        )  ## this training data is handgated cells which will also split futher into train and test set
        trainHandgated(
            ExprSet,
            train.X,
            relevantMarkers,
    ...
    for c, f in enumerate(inputs):
        if os.path.exists(f):
            ...
            result, pp = CelltypePredPerSample(
                expr,
                SelfObject,
                relevantMarkers,
                indx2ct,
    ...
    if output.shape[0] > 0:
        ...
        name = (
            ...
            + str(method)
            + "_"
            + str("_Posterior_Probability_Prediction_Result.csv")
        )
        ...
        print("Accuracy reports saved in output directory")
